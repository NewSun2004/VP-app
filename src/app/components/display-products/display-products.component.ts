...
export class DisplayProductsComponent implements OnInit {
      ...
      Object.keys(params).forEach((paramKey) => {
        ...
        if (Array.isArray(selectedFilters)) {
          selectedFilters.forEach((filterName) => {
            const selectedFilter = filterType.find(
          ...
          const selectedFilter = filterType.find(
            (filter) => filter.name === selectedFilters
          );
          if (selectedFilter) {
            selectedFilter.selected = true;
          }
  ...
  updateFilterName(): void {
    ...
    if (rawRouteUrl.includes('Male') && !rawRouteUrl.includes('Female')) {
      ...
      rawRouteUrl.includes('Female') &&
      !rawRouteUrl.includes('Male')
    ) {
    ...
    this.productName =
      routeBaseUrl.charAt(0).toUpperCase() + routeBaseUrl.slice(1);
  }
